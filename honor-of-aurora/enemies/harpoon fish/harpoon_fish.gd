...
## Дальний бой: гарпун (отдельная сцена); на теле — attack (замах HarpoonFish_Throw), не клип throw с гарпуном на рыбе.

const HARPOON := preload("res://enemies/harpoon fish/harpoon_projectile.tscn")

@export var projectile_scene: PackedScene = HARPOON
@export var projectile_spawn_delay: float = 0.42
