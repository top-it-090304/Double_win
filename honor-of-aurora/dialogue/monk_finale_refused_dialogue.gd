...
func ensure_lines_ready() -> void:
	...
	arr.append(_line("healer", "Ты остановился. Цепь не закрыта — и корона не получит своего акта. Мой контракт… остаётся контрактом. Письма дочери я буду читать без права ответить тем же телом, с тем же сроком."))
	arr.append(_line("healer", "Я не виню тебя. Клятва «не навреди» — она про чужих, кого я не вижу. А про свою кровь — она режет глубже любого скальпеля."))
	...
	arr.append(_line("healer", "Иди, если зовёт совесть. Останься в круге — если нужен только отвар. Я буду здесь."))
