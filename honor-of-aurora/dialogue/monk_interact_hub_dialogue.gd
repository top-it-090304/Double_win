...
func _build_story_question_option_dicts() -> Array:
	...
	opts.append({
		...
		"grant_flags": PackedStringArray([]),
		"continuation": [
			["healer", "По указу — зачистить острова от стражей и открыть глубокие жилы Сердцевины. Маяки на материке гаснут без этой руды, торговые пути слепнут. Официально ты — освободитель. Неофициально… это сложнее."],
		],
	})
	if StoryState.has_flag("intro_base_island_done"):
		opts.append({
			...
			"grant_flags": PackedStringArray([]),
			"continuation": [
				["healer", "Главный остров — лагерь экспедиции Ордена Тихой Зари. С причала ходят лодки к другим островам. Корабль ушёл: мы остались. Шахта под лагерем цепляет верхний слой той же системы, что тянется под островами."],
			],
		})
	...
	if StoryState.has_flag("boss_post_2_done"):
		opts.append({
			...
			"grant_flags": PackedStringArray([]),
			"continuation": [
				["healer", "Казначей нарисовал шахту, чтобы указ выглядел экономическим проектом. В реальности глубокая руда под островами недоступна, пока стоят стражи. Корона знала это заранее."],
			],
		})
	if StoryState.has_flag("boss_post_3_done"):
		opts.append({
			...
			"grant_flags": PackedStringArray([]),
			"continuation": [
				["healer", "Не богиня. Не демон. Сила — древнее всех слов о ней. Старые тексты ордена зовут её «первый свет в волне». Она дала архипелагу имя. Стражи держали её сон. С каждым убитым стражем сон тоньше. Что будет, когда она проснётся, — не знает никто."],
			],
		})
