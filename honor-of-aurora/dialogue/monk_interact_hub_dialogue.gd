	...
	if StoryState.has_flag("worker_youth_intro_done") and not StoryState.has_flag("worker_youth_recruited") and not StoryState.has_flag("worker_youth_dead"):
		opts.append({
			...
			"continuation": [
				...
				["healer", "Он думает, что поход — это слава и рассказы у огня. Нет. Поход — это грязь, кровь и крик, который не помещается в горло. Он не обучен, не закалён, не готов."],
