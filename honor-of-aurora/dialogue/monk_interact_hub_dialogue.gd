...
func _build_story_question_option_dicts() -> Array:
	...
	if StoryState.has_flag("intro_base_island_done"):
		opts.append({
			...
			"grant_flags": PackedStringArray([]),
			"continuation": [
				["healer", "Главный остров — лагерь экспедиции. С причала ходят лодки к берегам других островов — это твой путь, если бумага и ветер позволяют. Корабль экспедиции отчалил: впереди туман, острова и то, кто держит узлы. По голубиной почте сюда знали заранее, что пришлёт рыцаря по указу короля — и вот ты."],
			],
		})
