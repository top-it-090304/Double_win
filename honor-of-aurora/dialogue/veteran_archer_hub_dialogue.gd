...
func _build_story_option_dicts() -> Array:
	...
	opts.append({
		...
		"grant_flags": PackedStringArray([]),
		"continuation": [
			["veteran", "Указ прост: зачистить острова, открыть рудники. Маяки на материке гаснут — без сердцевины кораблям конец. Я не читал мелкий шрифт, но вижу, что происходит: купцы не плывут, хлеб дорожает, люди голодают. Это не философия — это арифметика."],
		],
	})
