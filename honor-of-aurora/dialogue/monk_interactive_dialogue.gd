...
func _build_monk_story_5() -> void:
	...
	if StoryState.has_flag("worker_youth_dead"):
		lines.append(_plain("healer", "Ты отдал за этот поход жизнь юноши. Я отдаю за него свою свободу. Мы оба заплатили тем, что не принадлежало нам. Разница в том, что его цена уже уплачена. Моя — ещё нет."))
