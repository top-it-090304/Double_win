...
func _build_monk_story_2() -> void:
	...
	lines.append(_plain("hero", "Ты говоришь это не как о чужой победе. Хочешь выговориться — я слушаю. Это не приказ."))
	lines.append(_plain("healer", "Тогда коротко. Моя жена умерла не от меча. От лихорадки, от нехватки рук, от того, что война забирает не только тех, кто в броне. Я приехал сюда уже вдовцом — думал, работа заглушит."))
...
func _build_monk_story_4() -> void:
	lines.append(_plain("hero", "С большой земли доходят вести. Твоё лицо не праздничное — хочешь об этом говорить?"))
	lines.append(_plain("healer", "Ты спросил. Письма начались не с объяснений. С одной фразы: «Я жива». Я читал это, как читают приговор — только наоборот: как отмену смерти."))
...
func _build_monk_story_5() -> void:
	lines.append(_plain("hero", "Ты всё ещё ждёшь дороги домой — туда, куда корона обещает отпуск после цепи?"))
	lines.append(_plain("healer", "Ты задал вопрос — отвечу. Она пишет, что ждёт на большой земле — там, где корона ещё помнит дороги без тумана. Я… я хочу доехать. Не «когда-нибудь». Когда цепь отпустит."))
