...
func ensure_lines_ready() -> void:
	...
	lines.append(_plain("healer", "Рудники на материке иссякают. Без Сердцевины маяки гаснут, торговые пути рушатся. То, что мы берём у базы, — верхняя жила, её мало. Королю нужен доступ к глубоким жилам под островами — а стражи его не пускали. Вот и весь указ."))
	...
	lines.append(_plain("healer", "Остался один страж. Последний замок. Если ты снимешь его — Заря начнёт просыпаться. Никто не знает, что будет: старые тексты расходятся. Но глубокие жилы откроются, Корона получит Сердцевину, а я получу дорогу к дочери."))
