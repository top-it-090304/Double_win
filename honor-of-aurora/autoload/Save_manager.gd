...
## Единый счётчик «ходов времени» после возвращения на базу с острова (в UI — дни); общий для срока приказа и прибытия каравана.
...
## Срок по счётчику истёк (0 осталось), но исход приказа и новый приказ фиксируются только после отправки/отпуска каравана этого рейса.
...
func apply_auto_phone_ui_settings() -> void:
	ui_scale_percent = 100
	touch_mode = 0
	touch_scale_percent = 100
	touch_opacity_percent = 52
