extends Node2D
## Минимальный тест лесоруба: герой + рабочий + дерево + зона + навигация + HUD.
## Запуск: F6 на `test_wood_worker_minimal.tscn`.
## Рабочий должен быть в группе `squad_member` — иначе атака рядом с ним не откроет меню приказов (так вешает GameManager).
